...
int ICM42688::getAGT() {
    ...
    _t = ((int16_t)((buffer[0] << 8) | buffer[1])) / 132.48f + 25;  // Temperature in Celsius

    _acc[0] = (int16_t)((buffer[2] << 8) | buffer[3]) * AccelSensitivity;  // Scale factor for accel
    _acc[1] = (int16_t)((buffer[4] << 8) | buffer[5]) * AccelSensitivity;
    _acc[2] = (int16_t)((buffer[6] << 8) | buffer[7]) * AccelSensitivity;

    _gyr[0] = (int16_t)((buffer[8] << 8) | buffer[9])   * GyroSensitivity;  // Scale factor for gyro
